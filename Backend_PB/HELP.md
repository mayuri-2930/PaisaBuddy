...
Use `./run-local.sh` from `Backend_PB/` for local development. It starts the backend on port `8081` by default and safely stops an older PaisaBuddy backend instance on that same port before relaunching.
